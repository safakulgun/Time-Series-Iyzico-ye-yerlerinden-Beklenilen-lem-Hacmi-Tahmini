...
# Görev 1 : Veri Setinin Keşfi
            # 1. iyzico_data.csv dosyasını okutunuz. transaction_date değişkeninin tipini date'e çevirelim.
            ...
            # 5.Her üye iş yerinin her bir yıl içerisindeki transaction count grafiklerini gözlemleyelim.
...
# Görev 3 : Modellemeye Hazırlık
            ...
            # 3.Veri setini train ve validation olarak ayıralım.

# Görec 4 : LightGBM Modelini oluşturunuz ve SMAPE ile hata değerini gözlemleyelim.
...
df = pd.read_csv("C:/Users/sfkul/OneDrive/Masaüstü/zaman serisi/iyzico_data.csv")
df.drop("Unnamed: 0", axis=1, inplace=True) #istemediğimiz kolonu veriden düşürüyoruz.
df.head()
df.tail()
...
for id in df.merchant_id.unique():
    ...
    plt.subplot(3, 1, 1, title = str(id) + ' 2018-2019 Transaction Count') #3 satırdan oluştur her birinde  bir  grafik
    df[(df.merchant_id == id) & ( df.transaction_date >= "2018-01-01" ) & (df.transaction_date < "2019-01-01")]["Total_Transaction"].plot()
    plt.xlabel('')
    plt.subplot(3, 1, 2,title = str(id) + ' 2019-2020 Transaction Count') #2. bir satırda görselleştirme
